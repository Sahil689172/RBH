...
function InertiaWord({
  ...
  const handlePointerEnter = () => {
    ...
    const xKick = clamp(
      withFallback(velocity.x * intensity * 3.4, direction * intensity * 28),
      -MAX_TRANSLATE,
      MAX_TRANSLATE,
    );
    const yKick = clamp(
      withFallback(velocity.y * intensity * 3.4, -intensity * 18),
      -MAX_TRANSLATE,
      MAX_TRANSLATE,
    );
    const rotateKick = clamp(
      withFallback(
        (velocity.x - velocity.y) * intensity * 0.85,
        direction * intensity * 16,
      ),
      -MAX_ROTATION,
      MAX_ROTATION,
    );
...
export default function TextInertia({
  ...
  return (
    <div
      ...
      {...props}
    >
      <span className="about-text-inertia-sr">{ariaLabel ?? text}</span>
