...
const createExplosion = ({ index, total }: { index: number; total: number }) => {
  ...
  const angle = total <= 1 ? 0 : (index / (total - 1)) * angleRange;
...
type TextExplodeProps = {
  text: string;
  className?: string;
  mode?: 'loop' | 'hover' | 'once';
};
...
export default function TextExplode({
  ...
  useEffect(() => {
    ...
    animateSequence();
  }, [characters.length, mode, animateSequence, controls]);
  ...
  return (
    <motion.div
      ...
      className={cn(
        'flex flex-wrap items-center justify-center tracking-normal',
